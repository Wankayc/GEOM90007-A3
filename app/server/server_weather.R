...
SENSOR_CSV <- here("data",
                   "raw",
                   "weather_and_air",
                   "microclimate-sensors-data.csv")
...
read_microclimate_daily <- function(path,
...
read_bom_range <- function(dir_path) {
  ym <- seq(lubridate::ymd("2024-09-01"),
            lubridate::ymd("2025-10-01"),
            by = "1 month")
...
bom_for_join <- bom_daily |>
  ...
  select(
    ...
    cl3,
    airtemperature_bom,
    relativehumidity_bom,
    averagewindspeed_bom,
    averagecloud_bom
  )
...
=======
      ...
      wind_ico <- if (is.na(wx$averagewindspeed_bom))
        ...
        "🍃"
      else if (wx$averagewindspeed_bom < 40)
        "💨"
      else
        "🌬️"
      pm_ico   <- if (is.na(wx$pm25_mean))
        ...
        "🔴"
      rain_ico <- if (!is.na(wx$rain) && wx$rain >= 5)
        "🌧️"
      else if (!is.na(wx$rain) && wx$rain >= 1)
        "🌦️"
      else
        "☀️"
>>>>>>> 091cdea30c4b1ee52ab7e22bc65041490429c785
      ...
      div(
        ...
        div(
          ...
          div(
            ...
            span(
              class = "val",
              style = sprintf("color:%s", temp_col),
              sprintf("%.0f°", coalesce(
                wx$airtemperature_bom, (wx$tmin + wx$tmax) / 2
              ))
            ),
            span(class = "unit", sprintf(
              "(%.0f°–%.0f°)",
              coalesce(wx$tmin, NA_real_),
              coalesce(wx$tmax, NA_real_)
            ))
          ),
          span(class = "cap", "Temp")
        ),
        ...
        div(
          ...
          div(
            ...
            span(class = "val", if (is.na(wx$rain))
              "–"
              else
                sprintf("%.0fmm", wx$rain))
          ),
        ...
        div(
          ...
          div(
            ...
            span(
              class = "val",
              if (is.na(wx$averagewindspeed_bom))
                "–"
              else
                sprintf("%.0f", wx$averagewindspeed_bom)
            ),
        ...
        div(
          ...
          div(
            class = "row1",
            span(class = "ico", "✨"),
            span(class = "val", if (is.na(wx$sun))
              "–"
              else
                sprintf("%.1fh", wx$sun))
          ),
        ...
        div(
          ...
          div(
            ...
            span(
              class = "val",
              if (all(is.na(c(
                wx$pm25_mean, wx$pm10_mean
              ))))
                "–"
              else
                sprintf(
                  "%s/%s",
                  if (is.na(wx$pm25_mean))
                    "–"
                  else
                    sprintf("%.0f", wx$pm25_mean),
                  if (is.na(wx$pm10_mean))
                    "–"
                  else
                    sprintf("%.0f", wx$pm10_mean)
                )
            )
          ),
    ...
    day_advice <- function(row, acts_labels) {
      ...
      if (!is.na(maxT) &&
          maxT < 12 &&
          any(acts_keys %in% c("swimming", "picnic", "hiking")))
...
<<<<<<< HEAD
      if (!is.na(pm25) && pm25 > 25 && is_outdoor)
=======
      if (!is.na(pm25) &&
          pm25 > 35 &&
          is_outdoor)
...
>>>>>>> Stashed changes
    ...
    output$range_summary <- renderUI({
      ...
      tagList(div(class = "summary-list", lapply(seq_len(nrow(
        rows
      )), function(i) {
        r <- rows[i, ]
        meanT <- coalesce(r$airtemperature_bom, (r$tmin + r$tmax) /
                            2)
        
        div(
          class = "row-card",
          div(class = "date", format(r$date, "%a %d %b")),
          div(class = "wx", span(weather_emoji(
            r$rain, r$tmax
          )), span(weather_label(
            r$rain, r$tmax
          ))),
          ...
          div(
            class = "chips summary-metrics",
            div(
              class = "chip stack",
              div(class = "row1", span(class = "ico", "🌡️"), HTML(
                sprintf(
                  ...
                  coalesce(r$tmin, NA_real_),
                  coalesce(r$tmax, NA_real_)
                )
              )),
              span(class = "cap", "Temp")
            ),
            div(
              ...
              div(
                class = "row1",
                span(class = "ico", "🌧️"),
                span(class = "val", if (is.na(r$rain))
                  "–"
                  else
                    sprintf("%.0fmm", r$rain))
              ),
            ...
            div(
              ...
              div(
                ...
                span(
                  class = "val",
                  if (is.na(r$averagewindspeed_bom))
                    "–"
                  else
                    sprintf("%.0f", r$averagewindspeed_bom)
                ),
                span(class = "unit", "km/h")
              ),
              span(class = "cap", "Wind")
            ...
            div(
              ...
              div(
                ...
                span(
                  class = "val",
                  if (all(is.na(c(
                    r$pm25_mean, r$pm10_mean
                  ))))
                    "–"
                  else
                    sprintf(
                      "%s / %s",
                      if (is.na(r$pm25_mean))
                        "–"
                      else
                        sprintf("%.0f", r$pm25_mean),
                      if (is.na(r$pm10_mean))
                        "–"
                      else
                        sprintf("%.0f", r$pm10_mean)
                    )
                )
              ),
              span(class = "cap", "PM2.5 / PM10")
    ...
    output$outdoor_list <- renderUI({
      ...
      rank_list(
        text = NULL,
        labels = outs,
        input_id = ns("pool_outdoor"),
        class = "rank-list chips pool",
        options = sortable_options(group = list(
          name = "act",
          pull = "clone",
          put = FALSE
        ))
...
=======
      div(class = "itinerary-row", lapply(ds, function(d) {
        row <- calendar_feed |> dplyr::filter(date == d) |> dplyr::slice(1)
        wx_ico <- if (nrow(row))
          weather_emoji(row$rain, row$tmax)
        else
          "⛅️"
        wx_txt <- if (nrow(row))
          weather_label(row$rain, row$tmax)
        else
          "Cloudy"
          ...
          if (length(relevant_themes) > 0) {
            ...
            if (nrow(matching_places) > 0) {
              activities <- lapply(1:nrow(matching_places), function(i) {
                ...
                time_slots <- c("9:00 AM - 11:00 AM",
                                "1:00 PM - 3:00 PM",
                                "4:00 PM - 6:00 PM")
                time_slot <- time_slots[min(i, length(time_slots))]
                
                if (!is.null(place$Theme) &&
                    place$Theme == "Food & Drink") {
